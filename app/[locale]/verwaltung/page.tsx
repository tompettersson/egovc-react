...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const data = await getCachedSectorPage('verwaltung');

  const title = locale === 'en' ? 'Public Administration' : 'Verwaltung';
  const ogLocale = locale === 'en' ? 'en_US' : 'de_DE';

  return {
    title: data?.seo?.metaTitle || `${title} | EGovC`,
    description: data?.seo?.metaDescription || (locale === 'en'
      ? 'Digitization of public administration. E-Government solutions for municipalities, cities and authorities.'
      : 'Digitalisierung der öffentlichen Verwaltung. E-Government Lösungen für Kommunen, Städte und Behörden.'),
    openGraph: {
      title: data?.seo?.metaTitle || `${title} | EGovC`,
      description: data?.seo?.metaDescription || (locale === 'en'
        ? 'E-Government and digitization for public administration.'
        : 'E-Government und Digitalisierung für die öffentliche Verwaltung.'),
      type: 'website',
      locale: ogLocale,
    },
...
export default async function VerwaltungPage(props: Props) {
  const { locale } = await props.params;
  const dict = getDictionary(locale as Locale);

  // Fetch Payload data with fallback to hardcoded data
  const [payloadData, references] = await Promise.all([
    getCachedSectorPage('verwaltung'),
    getCachedReferences(),
  ]);
  const fallbackData = pagesData.verwaltung;

  // Use Payload data if available, otherwise fall back to hardcoded data
  const data = payloadData ? {
    hero: {
      title: payloadData.hero?.title || fallbackData.hero.title,
      subtitle: payloadData.hero?.subtitle || fallbackData.hero.subtitle,
      backgroundImage: fallbackData.hero.backgroundImage, // TODO: add Payload image support
    },
    intro: {
