...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const data = await getCachedSectorPage('gesundheitswesen');

  const title = locale === 'en' ? 'Healthcare' : 'Gesundheitswesen';
  const ogLocale = locale === 'en' ? 'en_US' : 'de_DE';

  return {
    title: data?.seo?.metaTitle || `${title} | EGovC`,
    description: data?.seo?.metaDescription || (locale === 'en'
      ? 'Digitization in healthcare. Solutions for hospitals, clinics and medical facilities.'
      : 'Digitalisierung im Gesundheitswesen. Lösungen für Krankenhäuser, Kliniken und medizinische Einrichtungen.'),
    openGraph: {
      title: data?.seo?.metaTitle || `${title} | EGovC`,
      description: data?.seo?.metaDescription || (locale === 'en'
...
export default async function GesundheitswesenPage(props: Props) {
  const { locale } = await props.params;
  const dict = getDictionary(locale as Locale);

  const [payloadData, references] = await Promise.all([
    getCachedSectorPage('gesundheitswesen'),
    getCachedReferences(),
  ]);
  const fallbackData = pagesData.gesundheitswesen;
